...
- The `CNAME` file names the custom domain `wickstacks.lifelift.app`, which is **live**:
  domain verified, certificate approved, HTTPS enforced (GitHub Pages API), and the org
  default domain (`lifelift.github.io/app-sites/`) now **301-redirects** there rather
  than serving content (both probed 2026-08-29, W35 Director pass). Links between pages
  stay **relative** anyway: if the custom domain is ever removed, serving falls back to
  the `/app-sites/`-prefixed default domain, and relative links survive both shapes.
