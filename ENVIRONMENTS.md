...
| Custom domain `wickstacks.lifelift.app` | **not yet** — the `CNAME` file is committed; the DNS record and the Pages custom-domain setting are the owner's | none yet | — | until DNS exists, requests to that host do not reach this site at all; the org default Pages domain serves it instead |
